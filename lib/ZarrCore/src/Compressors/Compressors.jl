...
"""
    DEFAULT_COMPRESSOR_FACTORY

A Ref holding a zero-argument function that returns the default compressor.
Override by setting `ZarrCore.DEFAULT_COMPRESSOR_FACTORY[] = () -> MyCompressor()`.
"""
const DEFAULT_COMPRESSOR_FACTORY = Ref{Function}(() -> NoCompressor())

"""
    default_compressor()

Return the default compressor. By default returns `NoCompressor()`.
Override by setting `ZarrCore.DEFAULT_COMPRESSOR_FACTORY[] = () -> MyCompressor()`.
"""
default_compressor() = DEFAULT_COMPRESSOR_FACTORY[]()
