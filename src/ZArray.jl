...
struct ZArray{T,N,M<:AbstractMetadata{T,N},S<:AbstractStore} <: AbstractDiskArray{T,N}
  metadata::M
  storage::S
  path::String
  attrs::Dict
...
function metadata_and_attrs(s, path, fill_as_missing, ::V2; metadata_str=nothing)
  metadata = getmetadata(s, path, fill_as_missing)
  attrs = getattrs(s, path)
  metadata, attrs
end

function ZArray(s::T, mode="r", path="", version=V2(); fill_as_missing=false, metadata_str=nothing) where {T<:AbstractStore}
  metadata, attrs = metadata_and_attrs(s, path, fill_as_missing, version)
  writeable = mode == "w"
