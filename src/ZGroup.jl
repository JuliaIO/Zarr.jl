...
function ZGroup(s::T, mode="r", path="", version=V2(); fill_as_missing=false, metadata_str=nothing) where {T<:AbstractStore}
  arrays = Dict{String,ZArray}()
  groups = Dict{String,ZGroup}()

  for d in subdirs(s, path)
    dshort = split(d, '/')[end]
    m = zopen_noerr(s, version, mode, path=_concatpath(path, dshort), fill_as_missing=fill_as_missing)
    if isa(m, ZArray)
      arrays[dshort] = m
    elseif isa(m, ZGroup)
      groups[dshort] = m
  ...
  attrs = getattrs(version, s, path, metadata_str)
...
is returned instead. 
"""
function zopen_noerr(s::AbstractStore, version::V2, mode="r";
  consolidated=false,
  path="",
  fill_as_missing)
  ...
  if is_zarray(s, path, version)
    return ZArray(s, mode, path, version; fill_as_missing=fill_as_missing)
  elseif is_zgroup(s, path, version)
    return ZGroup(s, mode, path, version; fill_as_missing=fill_as_missing)
  else
    return nothing
  end
end

function zopen_noerr(s::AbstractStore, version::V3, mode="r";
  consolidated=false,
  path="",
  fill_as_missing)
  consolidated && @warn("Consolidated stores not yet supported in zarr v3")
  metadata_str = StringView(s[path, "zarr.json"])
  nodeinfo = NodeInfo(metadata_str)
  if is_zarray(nodeinfo, version)
    return ZArray(s, mode, path, version; fill_as_missing=fill_as_missing, metadata_str)
  elseif is_zgroup(nodeinfo, version)
    return ZGroup(s, mode, path, version; fill_as_missing=fill_as_missing, metadata_str)
  else
    return nothing
  end
end
...
function zopen(s::AbstractStore, mode="r";
  ...
  if version === nothing
    version = determine_zarr_version(s, path)
  end
  r = zopen_noerr(s, version, mode; consolidated=consolidated, path=path, fill_as_missing=fill_as_missing)
  if r === nothing
