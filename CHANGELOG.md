...
- Add `ZarrTrimmable`, an opt-in subpackage for programs compiled with `juliac --trim=safe`. `@zarr_reader(dtypes, ndims, codecs, stores; tag, path_store, read_strategy)` generates a closed-set reader that adds `zopen(f, reader, path)`, `zopen(reader, path)` (at most 3 dtype x ndims x zarr_format variants) and `zcreate(reader, path, T, dims; ...)` methods over a `Union`-typed codec pool and store pool; `ArrayMeta` with `read_meta`/`write_meta` is an `Any`-free mirror of `.zarray`. `ZarrTrimmable` is not re-exported by `Zarr`.
- Add the `codec_id(::Type{C})` extension point to `ZarrCore` (the numcodecs id a compressor type is stored under; `""` for `NoCompressor`), implemented by `ZarrZstd`, `ZarrZlib` and `ZarrBlosc`, and a keyword-option constructor `CompressorJSON(id; cname, clevel, shuffle, blocksize, level, checksum)`.
- Add a statically typed Zarr v3 open. `zopen(T, Val(N), store, path; pipeline = P)` opens a `zarr.json` array with `P<:V3Pipeline` naming the whole codec chain, e.g. `V3Pipeline{Tuple{}, BytesCodec, Tuple{ZstdV3Codec}}`; the result is a fully concrete `ZArray{T,N,S,MetadataV3{T,N,P,ChunkKeyEncoding}}` and the path is juliac `--trim=safe` clean. It is built on an `Any`-free `zarr.json` schema (`ZarrJsonV3`, `CodecJSON`, `CodecConfigJSON`, `ChunkGridJSON`, `ChunkGridConfigJSON`, `ChunkKeyEncodingJSON`, `ChunkKeyEncodingConfigJSON`, the accessors `chunk_grid_json`, `chunk_key_encoding_json`, `codecs_json`, and `parse_zarrjson`), all `public`.
- Add the v3 codec extension points `Codecs.V3Codecs.codec_name(::Type{C})` (the spec name a codec type is stored under; the instance `name(c)` now falls back to it) and `Codecs.V3Codecs.getCodec(::Type{C}, ::CodecJSON, ctx)`, implemented by `ZarrZstd`, `ZarrZlib` and `ZarrBlosc` next to their v3 codecs, plus `Codecs.V3Codecs.pipeline_from_json(P, codecs, ctx)`, which validates a stored codec chain against a requested pipeline type. A codec joins a typed `V3Pipeline` once it defines both.
- `zcreate(...; zarr_format = 3)` is juliac `--trim=safe` clean: v3 array creation now writes a single `zarr.json` (with `attributes` included) through `write_new_metadata` instead of a read-modify-write. Writing a *sharded* v3 array from a compiled program is still not trim-clean.
- `@zarr_reader` gains `zarr_format` (`(2,)`, `(3,)` or `(2, 3)`; a bare integer is accepted) and `v3_codecs` (a non-empty tuple of `V3Codec{:bytes,:bytes}` types, required when 3 is in the pool). A v3 reader emits a `BBPool_tag` bytes->bytes codec pool and a `P_tag = V3Pipeline{Tuple{}, BytesCodec, Vector{BBPool_tag}}`, and its visitor tries `.zarray` then `zarr.json`. `codecs` is optional when 2 is not in the pool (no `zcreate` methods are emitted then).
- The statically typed `zopen(::Type{T}, ::Val{N}, ...)` is now a single keyword method taking `compressor` (v2) or `pipeline` (v3); exactly one of the two is required and giving neither or both is an `ArgumentError`. `compressor` is no longer a mandatory keyword of a separate method, but code that passed it keeps working unchanged.
- **Breaking for direct callers**: every Zarr v3 `JSON.lower` method (codecs, `lower_chunk_key_encoding`, `lower3`) now returns a `NamedTuple` instead of a `Dict{String,Any}`, so a lowered document is indexed with symbols (`lowered[:codecs]`) rather than strings. The JSON written to disk is unchanged.
- Fix `fill_as_missing = true` on Zarr v3 arrays, which errored with `cannot reinterpret UInt8 as Union{Missing,Float64}`: the v3 decode pipeline now strips `Missing` from the output element type before handing it to the array->bytes codec, as the v2 path already did.
- Fix the v3 `data_type` written for complex arrays: `ComplexF32`/`ComplexF64` are now stored as `"complex64"`/`"complex128"` (the spec names) instead of `"complexf32"`/`"complexf64"`, which Zarr.jl itself could not read back.
